...
API backend-приложение – для создания заявок и продуктов; разграничение прав доступа через групповые политики (основываясь на архитектуру RBAC) к элементам бизнес логики; аутентификая/авторизация пользователя.
...
├── docker-compose.yaml
├── Dockerfile
├── dump.sql                     # дамп с тестовыми данными
├── entrypoint.sh
├── manage.py
